...
if (array_key_exists("depth",$_GET))
 $dodepth=true; else $dodepth=false;
...
if ($dopage && $Rnds>0)
 $RndsPerPage=1*ceil($Rnds/ceil(($Rnds*$Rnds)/$pageconst));
...
if (($TotalVotes>0) && !$NoRunningTally)
{
 ...
 if ($dodepth || $SkipZero || ($Elim[$RndNum]==0 && $RndNum<$Rnds-1))
 { $ZeroSkip++; }
 else
 {
  PrintTitle();
  ?>
Round <?php echo $RndNum+1-$ZeroSkip; ?>

<table border=1 cellspacing=0 cellpadding=<?php echo $CellPad; ?>>
<?php 
 $qs="?b=0000000";
 for ($CndNum=0; $CndNum<=$Cands; $CndNum++)
 {
...
?>
<tr><td>
<img src="bar.php?width=8&height=8&color=<?php echo substr($ColorStr[$Sort[$CndNum]],1); ?>" alt="" border=0>
<?php
echo number_format($VoteMatrix[$RndNum][$Sort[$CndNum]],$Digits)." ";
    ...
    if ($Sort[$CndNum]==abs($Elim[$RndNum])-1)
    {
     ...
     else
     {
      if (($CandsLeft==$Seats && $RndNum>$LastElim) && $RndNum>0 &&
       ...
       echo "<br><b>Equalized</b>";
      else echo "<br><b>Elected</b>";
     }
    }
   }
   ?></td></tr><?php 
...
<img src="<?php echo "pie100.php".$qs ?>"
 alt=""> <br>
<?php
//final round
//------------------------Chart captions
if (($RndNum<$Rnds-1))
{ ?><p>Go to the <a href="<?php if($RndNum+1>=$RndEnd) echo $ThisFile."&amp;page=".($page+2); ?>#Round<?php echo ($RndNum+2); ?>">next round</a><br>.<?php }
...
?><p>
Also, view the
<a href="<?php echo $ThisFile."&amp;depth=1"; ?>">Ballot Depth</a> info to see how much lower rankings contributed to the tally.<p>
...
?>
<p><a href="<?php if($page>0) echo $ThisFile."&amp;page=1"; ?>#Round1">First Round</a><br> <?php 
  if ($RndNum<$Rnds-1)
  { ?><a href="<?php if($RndNum+1>=$RndEnd) echo $ThisFile."&amp;page=".($page+2); ?>#Round<?php echo $RndNum+2; ?>">Next Round</a><br> <?php } 
  ?><a href="<?php if($page<$LastPage) echo $ThisFile."&amp;page=".($LastPage+1); ?>#Round<?php echo $Rnds; ?>">Final Round</a><br> <?php 
  if ($RndNum>0)
  {?><a href="<?php if($RndNum-1<$RndStart) echo $ThisFile."&amp;page=".$page; ?>#Round<?php echo $PrevRnd; ?>">Previous Round</a><br> <?php }
?>
<a href="<?php echo $ThisFile."&amp;depth=1"; ?>">Ballot Depth</a><br>
<a href="<?php echo $InfoFile; ?>">How it works</a><br>
<a href="<?php echo $HomeFile; ?>">Main Page</a><br>
<?php
$PrevRnd=$RndNum+1;
} //zero skip
...
<img src="bar.php?width=8&height=8&color=<?php echo substr($ColorStr[$Sort[$CndNum]],1); ?>" alt="" border=0>
<?php
echo number_format($Depth[$i0]*100.0/$DepthSum,1)."% ";
echo $i0+1;
$Suffixnum=($i0+1)%10;
...
<img src="<?php echo "pie100.php".$qs ?>" alt=""><br>
<a href="<?php if($page>0) echo $ThisFile."&amp;page=1"; ?>#Round1">First Round</a><br>
<a href="#Round<?php echo $Rnds; ?>">Last Round</a><br>
<a href="<?php echo $InfoFile; ?>">How it works</a><br>
